# Practical 0 - Reproducible Research in R with the Tidyverse
## Sarah de Lima
### 2026-05-15
...
```{r}
rows_datasaur <- datasaurus_dozen %>%
  nrow()
cols_datasaur <- datasaurus_dozen %>%
  ncol()
cbind(rows_datasaur, cols_datasaur)
```
```{r}
...
These data points all have a similar range and spread, and all follow multiple patterns, meaning that they have nearly identical summary statistics. Descriptive statistics can paint a piece of the picture, but it's important to visualize the data to get a better understanding of the relationship between two variables. 
...
### a) 
```{r}
aq <- as.tibble(airquality)

aq <- aq %>% filter (!is.na(Ozone))
```
...
aq_filt %>%
  ...
  geom_point(aes(colour = month_name))+
  geom_smooth(method = "lm", aes(colour = month_name), se = F)
```
